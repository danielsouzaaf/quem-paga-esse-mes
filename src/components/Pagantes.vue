  ...
  <div class="pagantes">
    <div class="inline-block relative w-64 mt-5">
      <select v-model="servico" class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
        <option v-for="(s) in Object.keys(servicos)" :key="s" :value="s">{{ s.charAt(0).toUpperCase() + s.slice(1) }}</option>
      </select>
      ...
      <div class="mt-5 text-xl">
        ...
        <div class="lists">
          <ul class="nes-list is-disc">
            <li v-for="(mes, key) in meses" v-bind:key="key" v-bind:class="{ 'font-extrabold': key === mesAtual}">{{mes}} - {{pagantes[servicos[servico].mesesPagantes[key]].name}}</li>
          </ul>
        </div>
      </div>
    </div>
...
import servicos from '../assets/servicos'
...
export default {
  ...
  computed: {
    pagante() {
      return Object.entries(servicos).map(([servico, informacoesServico]) => {
        // console.log(servico, informacoesServico.mesesPagantes, pagadores)
        console.log(pagadores[informacoesServico.mesesPagantes[currentDate.getMonth()]])
        return {
          servico: servico,
          pagante: pagadores[informacoesServico.mesesPagantes[currentDate.getMonth()]]
        }
      }).reduce((acc, cur) => ({ ...acc, [cur.servico]: cur.pagante }), {})
    }
  }
}
</script>
